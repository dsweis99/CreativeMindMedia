...
export function CaseStudy() {
  ...
  const deliveryGridColumns = project?.sol.length === 3 ? 'lg:grid-cols-3' : 'lg:grid-cols-2';
  ...
  return (
    <div>
      ...
      <section className="bg-[var(--color-bg-secondary)] py-24 sm:py-32"><div className="mx-auto max-w-7xl px-6 md:px-8 lg:px-[60px]"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--color-accent)]">02 · The work</p><h2 className="mt-5 font-display text-5xl tracking-wide sm:text-6xl">What CMM delivered.</h2><div className={`mt-12 grid gap-5 sm:grid-cols-2 ${deliveryGridColumns}`}>{project.sol.map((service, index) => <div key={service} className="flex min-h-[320px] flex-col justify-between border border-[var(--color-border-subtle)] bg-[var(--color-card-bg)] p-8 transition-colors duration-300 hover:border-[var(--color-accent)]"><p className="font-display text-3xl text-[var(--color-accent)]">0{index + 1}</p><div className="flex items-center gap-3"><CheckCircle2 size={18} className="shrink-0 text-[var(--color-accent)]" /><p className="text-lg font-semibold">{service}</p></div></div>)}</div></div></section>
